...
OUT = r"C:\Users\jordi\Documents\GitHub\decodifica.net\public\og\blog\ia-organiza-pendientes.png"
...
# Title - auto-fit entre 88pt y 50pt
title = "5 IAs enchufadas"
title2 = "a tu semana"

font_size = 88
while font_size >= 50:
    title_font = ImageFont.truetype(FONT_BOLD, font_size)
    max_line_w = max(text_w(title, title_font)[0], text_w(title2, title_font)[0])
    if max_line_w <= W - 128:
        break
    font_size -= 4
...
sub = "Manus  ·  ChatGPT  ·  Claude  ·  Gemini  ·  Copilot"
sw, sh = text_w(sub, sub_font)
draw.text(((W - sw) // 2, y + 6), sub, font=sub_font, fill=GRAY)

# Bottom hint
hint_font = ImageFont.truetype(FONT_REG, 22)
hint = "Caso real: 4 horas  →  20 minutos"
hw, hh = text_w(hint, hint_font)
draw.text(((W - hw) // 2, y + 6 + sh + 18), hint, font=hint_font, fill=GRAY)
